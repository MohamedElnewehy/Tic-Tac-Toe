...
int AIGame::getEasyAIMove(const std::vector<char>& board)
{
    ...
    if (availableMoves.empty()) return -1; // if there aren't available places return -1
...
int AIGame::getMediumAIMove(const std::vector<char>& board, char aiPlayer, char humanPlayer)
{
    ...
    if (QRandomGenerator::global()->bounded(100) < 70) { //if number <70 play hard
...
int AIGame::getHardAIMove(const std::vector<char>& board, char aiPlayer, char humanPlayer)
{
    ...
    return minimax(tempBoard, 0, true, -1000, 1000, aiPlayer, humanPlayer);
}

int AIGame::minimax(std::vector<char>& board, int depth, bool isMaximizing, int alpha, int beta, char aiPlayer, char humanPlayer)
{
    int score = evaluate(board, aiPlayer, humanPlayer);

    // Terminal conditions
    if (score == 10 || score == -10) return score; // so one of them won
    if (getAvailableMoves(board).empty()) return 0; //draw
    ...
    if (isMaximizing) {
        ...
        for (int i = 0; i < 9; i++) {
            if (board[i] == ' ') {
                board[i] = aiPlayer;
                int moveScore = minimax(board, depth + 1, false, alpha, beta, aiPlayer, humanPlayer);
                board[i] = ' ';
        ...
        for (int i = 0; i < 9; i++) {
            if (board[i] == ' ') {
                board[i] = humanPlayer;
                int moveScore = minimax(board, depth + 1, true, alpha, beta, aiPlayer, humanPlayer);
                board[i] = ' ';
