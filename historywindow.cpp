    ...
    currentMoveIndex(0), replaySpeed(1000), isReplaying(false), isPaused(false)
{
    ...
    resize(1200, 800);                // ✅ Allows resize, maximize, minimize
    setMinimumSize(900, 600);         // ✅ Prevents from becoming too small
    setWindowFlags(Qt::Window);       // ✅ Keeps default window controls (OK)
...
void HistoryWindow::setupUI()
{
    ...
    // Back button
    QPushButton *backToMenuBtn = new QPushButton("⬅️ Back");
    backToMenuBtn->setFixedSize(100, 40);
    ...
    connect(backToMenuBtn, &QPushButton::clicked, this, &HistoryWindow::onBackToMenuClicked);
    mainLayout->addWidget(backToMenuBtn, 0, Qt::AlignLeft);

    // Filter and refresh section
    filterLayout = new QHBoxLayout();

    gameModeFilter = new QComboBox();
    gameModeFilter->setFixedHeight(40);
    gameModeFilter->addItem("All Games");
    gameModeFilter->addItem("Classic Mode");
    gameModeFilter->addItem("Overwrite Mode");
    ...
    rightLayout->addSpacing(-21);  // Moves the board up by 15px
...
void HistoryWindow::onBackToMenuClicked()
{
...
void HistoryWindow::loadGameHistory()
{
    try {
        ...
        // Handle all filter cases properly with exact matching
        for (const auto& game : gameHistory) {
            QString gameMode = QString::fromStdString(game.gameMode);

            // Apply comprehensive filter logic with exact matching
            bool shouldShow = false;

            if (filterText == "All Games") {
                shouldShow = true;
            } else if (filterText == "Classic Mode" && gameMode == "Classic Mode") {
                shouldShow = true;
            } else if (filterText == "Overwrite Mode" && gameMode == "Overwrite Mode") {
                shouldShow = true;
