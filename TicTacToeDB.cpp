...
TicTacToeDB::TicTacToeDB() {
    QFile dbTest("tictactoe.db");
    qDebug() << "Trying to open DB at:" << QDir::currentPath();
    ...
    // Create users table for login functionality with security questions
    executeSQL("CREATE TABLE IF NOT EXISTS users ("
               "id INTEGER PRIMARY KEY AUTOINCREMENT, "
               "username TEXT UNIQUE NOT NULL, "
               "password_hash TEXT NOT NULL, "
               "favorite_color TEXT NOT NULL, "
               "favorite_meal TEXT NOT NULL, "
               "favorite_word TEXT NOT NULL, "
               "birth_place TEXT NOT NULL, "
               "created_at DATETIME DEFAULT CURRENT_TIMESTAMP);");

    // Create session table for persistent login
    executeSQL("CREATE TABLE IF NOT EXISTS user_session ("
...
bool TicTacToeDB::createUser(const string& username, const string& password, const string& favoriteColor,
                             const string& favoriteMeal, const string& favoriteWord, const string& birthPlace) {
    // Input validation
    if (username.empty() || password.empty() || favoriteColor.empty() ||
        favoriteMeal.empty() || favoriteWord.empty() || birthPlace.empty()) {
        cerr << "All fields must be filled\n";
        return false;
    }
    ...
    sqlite3_stmt* stmt;
    string insertSql = "INSERT INTO users (username, password_hash, favorite_color, favorite_meal, favorite_word, birth_place) VALUES (?, ?, ?, ?, ?, ?)";
    ...
    sqlite3_bind_text(stmt, 3, sha256Hash(favoriteColor).c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 4, sha256Hash(favoriteMeal).c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 5, sha256Hash(favoriteWord).c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 6, sha256Hash(birthPlace).c_str(), -1, SQLITE_TRANSIENT);
...
bool TicTacToeDB::validateSecurityAnswers(const string& username, const string& favoriteColor,
                                          const string& favoriteMeal, const string& favoriteWord, const string& birthPlace) {
    sqlite3_stmt* stmt;
    string sql = "SELECT favorite_color, favorite_meal, favorite_word, birth_place FROM users WHERE username = ?";
    ...
    bool isValid = false;
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        string storedColor = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 0));
        string storedMeal = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 1));
        string storedWord = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 2));
        string storedPlace = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 3));

        isValid = (sha256Hash(favoriteColor) == storedColor &&
                   sha256Hash(favoriteMeal) == storedMeal &&
                   sha256Hash(favoriteWord) == storedWord &&
                   sha256Hash(birthPlace) == storedPlace);
    }
...
bool TicTacToeDB::updatePassword(const string& username, const string& newPassword) {
    ...
    string hashed = sha256Hash(newPassword);
...
void TicTacToeDB::saveUserSession(const string& username) {
    // Clear any existing session
    executeSQL("DELETE FROM user_session;");

    sqlite3_stmt* stmt;
    string sql = "INSERT INTO user_session (username) VALUES (?)";
...
string TicTacToeDB::getLastLoggedUser() {
    sqlite3_stmt* stmt;
    string sql = "SELECT username FROM user_session ORDER BY login_time DESC LIMIT 1";
...
void TicTacToeDB::clearUserSession() {
    executeSQL("DELETE FROM user_session;");
}

bool TicTacToeDB::hasActiveSession() {
    sqlite3_stmt* stmt;
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        hasSession = sqlite3_column_int(stmt, 0) > 0;
    }
