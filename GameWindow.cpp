...
void GameWindow::setupUI()
{
    ...
    for (int i = 0; i < 9; i++) {
        ...
        buttons[i]->setFixedSize(102, 102);
...
void GameWindow::initializeGame()
{
    ...
    } else if (gameMode == "Classic Mode" && classicGame) {
        classicGame->initializeGame();
        board = classicGame->getBoard();
        currentPlayer = classicGame->getCurrentPlayer();
        gameEnded = classicGame->isGameEnded();
        moveHistory = classicGame->getMoveHistory();
    } else {
...
void GameWindow::onCellClicked()
{
    ...
    if (makeMove(position, currentPlayer)) {
        ...
        if (!gameEnded && isAIGame && currentPlayer == aiPlayer) {
            ...
            aiTimer->start(1000); // 1 second delay
        }
    }
}
...
bool GameWindow::isValidMove(int position, char player)
{
    if (gameMode == "Overwrite Mode" && overwriteGame) {
        ...
        return classicGame->isValidMove(position);
    } else {
...
bool GameWindow::makeMove(int position, char player)
{
    if (gameMode == "Overwrite Mode" && overwriteGame) {
        ...
        if (classicGame->makeMove(position, player)) {
            board = classicGame->getBoard();
            currentPlayer = classicGame->getCurrentPlayer();
            gameEnded = classicGame->isGameEnded();
            moveHistory = classicGame->getMoveHistory();

            if (classicGame->checkWin(player)) {
                endGame(QString(player) + " Wins!");
            } else if (classicGame->checkDraw()) {
                endGame("Draw!");
            } else {
                updateStatus("Current Turn: " + QString(currentPlayer));
            }
...
void GameWindow::updateButtonDisplay(int position)
{
    ...
    if (gameMode == "Overwrite Mode" && overwriteGame) {
        ...
        cellValue = classicGame->getCellValue(position);
    } else {
...
void GameWindow::endGame(const QString& result)
{
    ...
    QTimer::singleShot(0, [this, enhancedMessage]() {
        QMessageBox msgBox(this);
        msgBox.setWindowTitle("🎮 Game Result");
        msgBox.setText(enhancedMessage);
        msgBox.setIcon(QMessageBox::Information);
...
void GameWindow::saveGameToDatabase()
{
    if (database && currentUserId != -1) {
        ...
        if (gameMode == "Overwrite Mode" && overwriteGame) {
            ...
            finalMoveHistory = classicGame->getMoveHistory();
        } else {
...
int GameWindow::getAIMove()
{
    if (aiGameModule) {
        if (gameMode == "Overwrite Mode" && overwriteGame) {
            // For overwrite mode, use a simplified strategy
            std::vector<int> validMoves;
            for (int i = 0; i < 9; i++) {
                if (overwriteGame->isValidMove(i, aiPlayer)) {
            ...
            if (!validMoves.empty()) {
                ...
                for (int move : validMoves) {
                    OverwriteGame tempGame = *overwriteGame;
                    if (tempGame.makeMove(move, aiPlayer) && tempGame.checkWin(aiPlayer)) {
                        return move;
                    }
                }
                ...
                for (int move : validMoves) {
                    OverwriteGame tempGame = *overwriteGame;
                    if (tempGame.makeMove(move, humanPlayer) && tempGame.checkWin(humanPlayer)) {
                        if (overwriteGame->isValidMove(move, aiPlayer)) {
                            return move;
                        }
                    }
                }
            ...
            return aiGameModule->getAIMove(board, aiPlayer, humanPlayer);
        }
    } else {
...
int GameWindow::getHardAIMove()
{
    if (gameMode == "Overwrite Mode" && overwriteGame) {
        ...
        for (int i = 0; i < 9; i++) {
            if (overwriteGame->isValidMove(i, aiPlayer)) {
                OverwriteGame tempGame = *overwriteGame;
                if (tempGame.makeMove(i, aiPlayer) && tempGame.checkWin(aiPlayer)) {
                    return i;
                }
            }
        }
        ...
        for (int i = 0; i < 9; i++) {
            if (overwriteGame->isValidMove(i, humanPlayer)) {
                OverwriteGame tempGame = *overwriteGame;
                if (tempGame.makeMove(i, humanPlayer) && tempGame.checkWin(humanPlayer)) {
                    if (overwriteGame->isValidMove(i, aiPlayer)) {
                        return i;
                    }
                }
        ...
        return minimax(tempBoard, 0, true);
    }
}

int GameWindow::minimax(std::vector<char>& board, int depth, bool isMaximizing)
{
    ...
    if (isMaximizing) {
        ...
        for (int i = 0; i < 9; i++) {
            if (board[i] == ' ') {
                board[i] = humanPlayer;
                int moveScore = minimax(board, depth + 1, true);
                board[i] = ' ';
